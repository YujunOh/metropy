...
class MetropyPreprocessor:
    ...
    def normalize_station_name(name: str) -> str:
        ...
        Examples:
            >>> MetropyPreprocessor.normalize_station_name("강남역")
            "강남"
            >>> MetropyPreprocessor.normalize_station_name("서울역(1호선)")
            "서울"
        """
        if pd.isna(name):
            return name
        ...
        # Remove 역 suffix
        name = re.sub(r'역$', '', str(name))
        
        # Remove content in parentheses (both Korean and English)
        name = re.sub(r'\([^)]*\)', '', name)
        name = re.sub(r'\[[^\]]*\]', '', name)
