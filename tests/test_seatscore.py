...
class TestSeatScoreEngine:
    ...
    def test_facility_weights(self, seatscore_engine):
        ...
        weights = seatscore_engine.FACILITY_WEIGHTS
        assert weights["에스컬레이터"] == 1.5
        assert weights["엘리베이터"] == 1.2
        assert weights["계단"] == 1.0

    def test_alpha_map(self, seatscore_engine):
        """시간대 배율이 올바르게 설정되었는지 확인"""
        alpha_map = seatscore_engine.ALPHA_MAP
    ...
    def test_get_alpha(self, seatscore_engine):
        ...
        # 퇴근 러시 (18-20시)
        assert seatscore_engine._get_alpha(19) == 1.3
        # 주간 (10-17시)
    ...
    def test_different_routes(self, seatscore_engine):
        ...
        # 긴 경로 vs 짧은 경로
        long_route = seatscore_engine.recommend("잠실", "신촌", hour, direction)
        short_route = seatscore_engine.recommend("강남", "역삼", hour, direction)
    ...
    def test_beta_penalty(self, seatscore_engine):
        ...
        assert seatscore_engine.BETA == 0.3
