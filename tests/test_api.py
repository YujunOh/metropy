...
class TestAPIEndpoints:
    ...
    def test_calibrate_get(self, test_client):
        ...
        assert data["facility_weights"]["에스컬레이터"] == 1.5
    ...
    def test_sensitivity_analysis(self, test_client, sample_route):
        """민감도 분석 엔드포인트 테스트"""
        response = test_client.post("/api/calibrate/sensitivity", json={
            "boarding": sample_route["boarding"],
            "destination": sample_route["destination"],
            "hour": sample_route["hour"],
        ...
        data = response.json()
        assert "beta_values" in data
        assert "car_scores_by_beta" in data

        # beta 값들이 올바른지 확인
        assert len(data["beta_values"]) == 6
        assert data["beta_values"][0] == 0.0
        assert data["beta_values"][-1] == 1.0
